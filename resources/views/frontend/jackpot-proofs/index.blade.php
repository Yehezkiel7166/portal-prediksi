...
<section class="bg-slate-950"><div class="mx-auto max-w-7xl px-4 py-12"><p class="mb-7 text-sm text-slate-400">Menampilkan <span class="font-semibold text-white">{{ $proofs->total() }}</span> bukti</p>
...
@if($loop->last)</div>@endif @empty<div class="rounded-xl border border-dashed border-slate-700 bg-slate-900 px-6 py-16 text-center"><h2 class="text-xl font-semibold text-white">Belum ada bukti jackpot</h2><p class="mt-3 text-sm text-slate-400">Bukti yang telah disetujui akan muncul di halaman ini.</p></div>@endforelse
