...
@endsection
@section('content')
<article data-theme-special="jackpot-detail" class="bg-slate-950"><div class="mx-auto max-w-4xl px-4 py-12"><a href="{{ route('jackpot-proofs.index') }}" class="text-sm font-semibold text-amber-400 hover:text-amber-300">← Kembali ke Bukti Jackpot</a><h1 class="mt-6 text-3xl font-bold text-white md:text-5xl">{{ $proof->title }}</h1><p class="mt-4 text-sm text-slate-500">Dipublikasikan {{ $proof->published_at->translatedFormat('d F Y H:i') }}</p><div data-theme-surface class="mt-8 overflow-hidden rounded-xl border border-slate-800 bg-slate-900"><img src="{{ asset('storage/'.$proof->image_path) }}" alt="{{ $proof->title }}" class="h-auto w-full"></div>@if(filled($proof->description))<div data-theme-rich-content class="mt-8 whitespace-pre-line text-base leading-8 text-slate-300">{{ $proof->description }}</div>@endif</div></article>
@endsection
